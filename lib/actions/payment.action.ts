...
export async function initializeTransaction(email: string, busId: string, selectedSeats: string[], location: string, luggage: {name:string, quantity: number}[]) {
  ...
  const totalLuggagePrice = await getLuggagePrice(luggage) || 0;
